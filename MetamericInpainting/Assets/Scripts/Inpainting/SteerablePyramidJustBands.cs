...
public class SteerablePyramidJustBands : CommandBufferCreator
{
    public bool hiq;
    ...
    public override void Init(int size, int pyramidDepth)
    {
        ...
        int nTextures = 1 + nbands;
        m_SteerablePyramid = new RenderTexture[nTextures];
        RenderTargetIdentifier[] steerIDS = new RenderTargetIdentifier[nTextures];
        ...
        m_CommandBufferSteerable.SetGlobalFloatArray("_Kernel", PyramidUtils.getFilter(0, true, false, 4));
        ...
        for (int i = 0; i < m_LODCount; i++)
        {
            for (int j = 0; j < nbands; j++)
            {
                ...
                m_CommandBufferSteerable.SetGlobalFloatArray("_Kernel", PyramidUtils.getFilter(2+j, hiq, combinedMatrix,nbands));
                m_CommandBufferSteerable.SetGlobalInt("_KernelWidth", PyramidUtils.getFilterWidth(2+j,hiq,combinedMatrix, nbands));
                ...
                if (preFilter && !combinedMatrix)
                {
                    ...
                    m_CommandBufferSteerable.CopyTexture(tempOutput2, 0, 0, steerIDS[1+j], 0, i);
                }
                else
                {
                    m_CommandBufferSteerable.Blit(loOutput, tempOutput, convolutionMaterial);
                    m_CommandBufferSteerable.CopyTexture(tempOutput, 0, 0, steerIDS[1+j], 0, i);
            ...
            m_CommandBufferSteerable.SetGlobalInt("_do2x2", hiq ? 0 : 1);
            m_CommandBufferSteerable.SetGlobalFloat("_LOD", (float)i);
