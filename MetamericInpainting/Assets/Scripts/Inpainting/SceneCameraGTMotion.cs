...
public class SceneCameraGTMotion : WarpFrameSource
{
    ...
    private Shader m_ShaderMotionWarp;
    public Shader shaderMotionWarp
    {
        get
        {
            if (m_ShaderMotionWarp == null)
                m_ShaderMotionWarp = Shader.Find("Hidden/MotionShaderWarp");

            return m_ShaderMotionWarp;
        }
    }

    private Material m_MaterialMotionWarp;
    public Material materialMotionWarp
    {
        get
        {
            if (m_MaterialMotionWarp == null)
            {
                if (shaderMotionWarp == null || shaderMotionWarp.isSupported == false)
                    return null;

                m_MaterialMotionWarp = new Material(shaderMotionWarp);
            }

            return m_MaterialMotionWarp;
        }
    }
    ...
    int width, height;
    public float motionMultiple = 1.5f;

    public RenderTexture tmpMotion;
    ...
    void Start()
    {
        ...
        tmpMotion = new RenderTexture(width, height, 32, RenderTextureFormat.ARGBFloat);
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        {
            materialMotion.SetFloat("motionMultiple", motionMultiple);
        ...
        {
            materialMotionWarp.SetTexture("currentMotion", motion);
            Graphics.Blit(source, tmpMotion, materialMotionWarp);
            Graphics.Blit(tmpMotion, motion);
        }    
