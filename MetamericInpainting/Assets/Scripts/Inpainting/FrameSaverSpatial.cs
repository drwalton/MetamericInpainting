...
public class FrameSaverSpatial : MonoBehaviour
{
    public WarpFrameSource warpFrameSource = null;
    public WarpTransformCommandBufferCreator warper = null;
    ...
    void Update()
    {
        ...
        if ((saveFrames && Time.frameCount >= 1) || Input.GetKeyUp(KeyCode.O))
        {
            // Subtract 1 here because we're actually saving the previous frame.
            string prefix = "Saved_Frame_" + (Time.frameCount - 1).ToString();
            ...
            if (warpFrameSource != null)
            {   
                //SaveRenderTextureRGBPNG(warpFrameSource.color, prefix + "_GT.png");
            }
