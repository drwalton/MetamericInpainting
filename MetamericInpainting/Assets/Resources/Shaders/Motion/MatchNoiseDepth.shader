...
#include "..\Utils.cginc"
	...
	float4 fragment(in Varyings input) : SV_Target
	{
		...
		float4 depth = _DepthTex.SampleLevel(my_point_clamp_sampler, input.uv, 0);
		//The depth is 0-0.5 for invalid, and 0.5->1 for invalid. So we map them 
		float d = depth.a < 0.5f ? depth.a * 2 : (depth.a - 0.5f) * 2;
		float2 shiftedUV = (2 * input.uv) - float2(1, 1);
		float4 projectedPoint = float4(shiftedUV.x, -shiftedUV.y, d, 1);
		projectedPoint = mul(warpMatrix, projectedPoint);
		shiftedUV = (projectedPoint + float2(1,1))*0.5f;
		//uv goes to 0-1
		//sample
		if (_useLUT == 0)
